...
CONFIG = {
    "start_year": 2020, "end_year": 2020, "base_url": "https://apps.irs.gov/pub/epostcard/990/xml",
    "base_dir": Path("./irs_990_data"), "db_name": "irs_990.sqlite", "max_download_workers": 4,
    "max_parsing_workers": os.cpu_count(), "filings_to_process_per_year": None,
}
...
class Form990Schema:
    ...
    FIELD_XPATH_MAP = {
        'ein': './/efile:Filer/efile:EIN', 'tax_year': './/efile:ReturnHeader/efile:TaxYr', 'org_name': './/efile:Filer/efile:BusinessName/efile:BusinessNameLine1Txt',
        'city': './/efile:Filer/efile:USAddress/efile:CityNm', 'state': './/efile:Filer/efile:USAddress/efile:StateAbbreviationCd', 'zip_code': './/efile:Filer/efile:USAddress/efile:ZIPCd',
        'total_revenue': {'IRS990': './/efile:CYTotalRevenueAmt', 'IRS990EZ': './/efile:TotalRevenueAmt', 'IRS990PF': './/efile:TotalRevAndExpnssAmt'},
        'total_expenses': {'IRS990': './/efile:CYTotalExpensesAmt', 'IRS990EZ': './/efile:TotalExpensesAmt', 'IRS990PF': './/efile:TotalExpensesRevAndExpnssAmt'},
        'total_assets_eoy': {'IRS990': './/efile:TotalAssetsEOYAmt', 'IRS990EZ': './/efile:TotalAssetsEOYAmt', 'IRS990PF': './/efile:TotalAssetsEOYAmt'},
        'contributions_grants': {'IRS990': './/efile:CYContributionsGrantsAmt', 'IRS990EZ': './/efile:ContributionsGiftsGrantsEtcAmt', 'IRS990PF': './/efile:ContriRcvdRevAndExpnssAmt'}
    }
    @classmethod
...
def clean_dataframes(filings_df: pd.DataFrame, grants_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not filings_df.empty:
        ...
        filings_df = filings_df.replace("", np.nan)
    if not grants_df.empty:
        ...
        grants_df = grants_df.replace("", np.nan)
...
def parse_local_xml_file(xml_path: Path, filing_year: int) -> Optional[Dict]:
    try:
        ...
        for grant in grants_data: grant.update({'filer_ein': filing_data.get('ein'), 'filing_year': filing_year})
...
def discover_and_download_zips(year: int) -> List[Path]:
    ...
    for i in range(1, 20):
        ...
        try:
            ...
            if response.status_code == 200: urls_to_download.append(url)
            else: break
    ...
    with ThreadPoolExecutor(max_workers=CONFIG['max_download_workers']) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(urls_to_download), desc=f"Downloading ZIPs {year}"):
            if result := future.result(): downloaded_paths.append(result)
...
def process_year(year: int) -> bool:
    ...
    if not all_filings_data:
        ...
        return True
    filings_df, grants_df = pd.DataFrame(all_filings_data), pd.DataFrame(all_grants_data)
...
def save_outputs(filings_df: pd.DataFrame, grants_df: pd.DataFrame, year: int):
    ...
    if not grants_df.empty: grants_df.to_csv(output_dir / f"grants_{year}.csv", index=False)
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if not grants_df.empty: grants_df.to_sql('grants', conn, if_exists='append', index=False)
    ...
    except Exception as e: logger.error(f"Failed to write to database: {e}")
