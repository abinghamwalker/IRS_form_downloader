...
def generate_profile_report(
    ...
    """Print a data quality and summary report to the console."""
    print("\n" + "=" * 80)
    ...
    if filings_df.empty:
        ...
        print(
            filings_df[["total_revenue", "total_expenses", "total_assets_eoy"]]
            .describe()
            .apply(lambda s: s.apply("{:,.2f}".format))
        )
    ...
    if grants_df.empty:
        ...
        # Filter out potential zero or negative amounts for a meaningful summary
        positive_grants = grants_df[grants_df["grant_amount"] > 0]
        if not positive_grants.empty:
            print(
                positive_grants[["grant_amount"]]
                .describe()
                .apply(lambda s: s.apply("{:,.2f}".format))
            )
        else:
...
def main():
    ...
    try:
        if args.source == "csv":
            ...
            grants_df = (
                pd.read_csv(grants_path) if grants_path.exists() else pd.DataFrame()
            )
        else:  # sqlite
            with sqlite3.connect(DB_PATH) as conn:
                query = f"SELECT * FROM filings WHERE filing_year = {args.year}"
                filings_df = pd.read_sql_query(query, conn)
                query = f"SELECT * FROM grants WHERE filing_year = {args.year}"
                grants_df = pd.read_sql_query(query, conn)
    except FileNotFoundError:
        print(
            f"ERROR: Data files for year {args.year} not found. "
