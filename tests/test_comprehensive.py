...
class TestForm990Parser:
    ...
    def test_unknown_form_type(self):
        xml_content = (
            ...
            "<ReturnData><IRSUnknownForm/></ReturnData></Return>"
        )
    ...
    def test_various_invalid_numeric_values(self, invalid_amount):
        xml = f"""<Return xmlns="http://www.irs.gov/efile">
          <ReturnData><IRS990PF>
            <TotalRevAndExpnssAmt>{invalid_amount}</TotalRevAndExpnssAmt>
          </IRS990PF></ReturnData>
        </Return>"""
    ...
    def test_very_large_financial_values(self):
        xml = """<Return xmlns="http://www.irs.gov/efile">
          <ReturnData><IRS990PF>
            <TotalRevAndExpnssAmt>5000000000</TotalRevAndExpnssAmt>
          </IRS990PF></ReturnData>
        </Return>"""
        parser = Form990Parser(xml)
    ...
    def test_negative_financial_values(self):
        xml = """<Return xmlns="http://www.irs.gov/efile">
          <ReturnData><IRS990PF>
            <TotalRevAndExpnssAmt>-50000</TotalRevAndExpnssAmt>
          </IRS990PF></ReturnData>
        </Return>"""
        parser = Form990Parser(xml)
    ...
    def test_missing_critical_fields(self):
        xml = """<Return xmlns="http://www.irs.gov/efile">
          <ReturnHeader><Filer></Filer></ReturnHeader>
          <ReturnData><IRS990PF/></ReturnData>
        </Return>"""
        parser = Form990Parser(xml)
...
class TestGrantParsing:
          ...
          <ReturnData><IRS990PF>
            <GrantOrContributionPdDurYrGrp>
              <RecipientBusinessName>
                <BusinessNameLine1Txt>INTL ORG</BusinessNameLine1Txt>
              </RecipientBusinessName>
    ...
    def test_grants_missing_optional_fields(self):
        xml = """<Return xmlns="http://www.irs.gov/efile">
          <ReturnData><IRS990PF>
            <GrantOrContributionPdDurYrGrp><Amt>5000</Amt></GrantOrContributionPdDurYrGrp>
          </IRS990PF></ReturnData>
        </Return>"""
        parser = Form990Parser(xml)
        grants = parser.parse_grants()
    ...
    def test_parse_form_with_no_grants(self):
        xml = (
            '<Return xmlns="http://www.irs.gov/efile">'
            "<ReturnData><IRS990PF/></ReturnData></Return>"
        )
        parser = Form990Parser(xml)
        grants = parser.parse_grants()
        assert grants == []
...
class TestDataCleaning:
    ...
    def test_cleaning_grants_dataframe(self):
        grants_df = pd.DataFrame(
            [
                {
                    "recipient_name": " lower case org ",
                    "recipient_state": "california",
                    "grant_purpose": "  education  ",
                }
            ]
        )
...
class TestSchemaValidation:
    ...
    def test_filing_dataframe_has_required_columns(self):
        ...
        filings_df, _ = clean_dataframes(
            pd.DataFrame([result["filing"]]), pd.DataFrame()
        )
...
class TestFileIOAndDatabase:
    ...
    def test_parse_xml_file_with_unicode_content(self, tmp_path):
        xml_content = """<Return xmlns="http://www.irs.gov/efile">
          ...
          <BusinessName>
            <BusinessNameLine1Txt>Café München</BusinessNameLine1Txt>
          </BusinessName>
    ...
    def test_database_index_creation(self, tmp_path):
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                filings_indexes = conn.execute(
                    "PRAGMA index_list('filings');"
                ).fetchall()
...
class TestWithRealFixtures:
    ...
    def test_sample_990_complete_parsing(self):
        ...
        assert len(result["grants"]) == 1
        assert result["grants"][0]["recipient_name"] == "HOMELESS SHELTER"
        assert result["grants"][0]["grant_amount"] == 25000.0
